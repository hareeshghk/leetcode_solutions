...
public:
    long long countSubarrays(vector<int>& nums, int minK, int maxK) {
        int n = nums.size();
        long long answer = 0;
        int acceptableStart = 0;
        int lastminkoccurrence = -1;
        int lastmaxkoccurrence = -1;
        for (int i = 0; i < n; ++i) {
            if (nums[i] == minK) {
                lastminkoccurrence = i;
            }

            if (nums[i] == maxK) {
                lastmaxkoccurrence = i;
            }
            ...
            if (lastminkoccurrence < acceptableStart || lastmaxkoccurrence < acceptableStart) continue;

            if (lastminkoccurrence == -1 || lastmaxkoccurrence == -1) continue;

            answer = answer + max(0, min(lastminkoccurrence, lastmaxkoccurrence) - acceptableStart + 1);
        }
        return answer;
        // int n = nums.size();
